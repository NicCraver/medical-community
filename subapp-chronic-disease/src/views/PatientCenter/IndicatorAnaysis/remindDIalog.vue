  ...
  methods: {
    ...
    /* async getData() {
      try {
        ...
        let { code, result } = await queryPatRemindSetting(params);
        if (code === 0) {
          console.log("获取设置", result);
          let remindWay =
            result?.remindWay?.indexOf(",") > -1
              ? result.remindWay.split(",")
              : result?.remindWay
              ? [result?.remindWay]
              : [];
          this.formData = {
    ...
    async getTemData() {
      let classFy =
        this.formData.remindType === "1"
          ...
          ? this.remindTypeL
          : "";
          console.log(`this.formData.remindType`,this.formData.remindType)
          console.log(`this.remindTypeD`,this.remindTypeD)
      try {
        let params = {
          classFy,
        ...
        if (code === 0) {
          ...
          if (result?.length) {
            this.$set(this.formData, "templateId", result[0].id);
            if (result?.length) {
              this.templateChange(result[0].id);
            }
          }
        }
      } catch (error) {}
    ...
    getMsgContent() {
      ...
      let str = cont.replace(/patName/g, this.userInfo?.name);
      this.msgContent = str;
    },
    // 保存
    sureFuc() {
        ...
        if (valid) {
          try {
            ...
            let params = {
              ...
              createUserId: sessionStorage.getItem("userId") || "", //创建人id
              createUserName: sessionStorage.getItem("loginName") || "", //创建人名称
            };
