   ...
   instance.interceptors.response.use(
     (res) => {
       ...
       console.log(
         `%c${"[HTTP]"} %c${dayjs().format(
           "HH:mm:ss"
         )} %c${res.config.method.toLocaleUpperCase()} %c${res.config.url}(%c${
           res.status
         }` + "%c)",
         "font-weight:bold",
         "font-weight:bold",
     ...
     (err) => {
       console.log('anx-core-------',err)
       let config = err.config;
       let errres = err.response;
       let err_code = errres?.status ?? 0;
       let err_msg = "";
       // 收集错误信息
       switch (err_code) {
         case 400:
       ...
       //   `%c${"[HTTP]"} %c${dayjs().format(
       //     "HH:mm:ss"
       //   )} %c${errres.config.method.toLocaleUpperCase()} %c${
       //     errres.config.url
       //   }(%c${errres.status}` + "%c)",
       //   "font-weight:bold",
       //   "font-weight:bold",
       ...
       if (config.__retryCount >= config.retry) {
         ...
         if (responseInterceptorErrorCb) {
           const _res = responseInterceptorErrorCb(
             { ...err },
             err_code,
             err_msg
           );
