            ...
            <div>
              <a-breadcrumb>
                <a-breadcrumb-item v-for="v in state.breadcrumbData" :key="v.path">
                  <a v-if="v.meta.breadcrumb" @click="menuGoPage(v)" href="javascript:void(0);">{{ v.meta.title }}</a>
    ...
    <template #menuHeaderRender>
      ...
      <div v-if="!state.collapsed">
        <img
          ...
          "
        />
        <img v-if="store.state.app.hosName === '鹰眼观谱科技'" src="../assets/meunlogo2.png" />
        <img v-if="store.state.app.hosName === '阿克苏'" src="../assets/akesulogos.jpg" />
        <img v-if="store.state.app.hosName === '齐齐哈尔'" src="../assets/qiqihaer-logo.png" />
...
watch(
  () => store.state.app.menuData,
  (newValue, oldValue) => {
    console.log(`store.state.app.menuData`, newValue)
    if (isHei()) {
    ...
    state.selectedKeys = []
    state.openKeys = []
    state.breadcrumbData = getBreadcrumbById(menuDataKeys.value, (node) => node.path === str)
...
watch(
  ...
  (newValue, oldValue) => {
    ...
    state.breadcrumbData = getBreadcrumbById(menuDataKeys.value, (node) => node.path === newValue)
...
onMounted(() => {
  console.log(`onMountedstore.state.app.menuData`, store.state.app.menuData)
  if (isHei()) {
    state.collapsed = true
    proConfig.value.menuHeaderRender = state.collapsed ? false : undefined
  }
...
function menuAssignment() {
  console.group('生产调试')
  console.log(`menuDataKeys===`, `menuDataKeys${process.env.VUE_APP_IDENTIFICATION}`)
  console.log(`isHei()`, isHei())
  if (isHei()) {
    menuDataKeys.value = JSON.parse(sessionStorage.getItem(`menuDataKeys${process.env.VUE_APP_IDENTIFICATION}`))
    menuData.value = JSON.parse(sessionStorage.getItem(`menuDataKeys${process.env.VUE_APP_IDENTIFICATION}`))
  } else {
...
function commonRoutePath(item) {
  ...
  state.selectedKeys = []
  state.openKeys = []
  state.breadcrumbData = getBreadcrumbById(menuDataKeys.value, (node) => node.path === path)
