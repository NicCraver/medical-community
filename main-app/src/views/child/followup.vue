...
const url = `${config.followup}/followup/`
const baseroute = process.env.BASE_URL + 'app-followup'

let microAppData = { basePath: '/app-followup', msg: '来自基座的数据' }
function handleCreate() {
  console.log('app-followup 创建了')
  store.dispatch('app/setAppLoading', true)
}

function handleBeforeMount() {
  console.log('app-followup 即将被渲染')
}

function handleMount() {
  console.log('app-followup 已经渲染完成')
  store.dispatch('app/setAppLoading', false)
}

function handleUnmount() {
  console.log('app-followup 卸载了')
  store.dispatch('app/setAppLoading', false)
}

function handleError() {
  console.log('app-followup 加载出错了')
  store.dispatch('app/setAppLoading', false)
}

function handleDataChange(e) {
  // 接收子应用传递的数据 e.detail.data
  console.log('来自子应用 app-followup 的数据:', e.detail.data)
