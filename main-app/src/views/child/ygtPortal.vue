 ...
 * @LastEditors: Nic
 * @Description: 
 * @FilePath: /medical-community/main-app/src/views/child/ygtPortal.vue
-->
<template>
  <micro-app
    name="app-ygtPortal"
...
function handleCreate() {
  console.log('app-ygtPortal 创建了')
  store.dispatch('app/setAppLoading', true)
}

function handleBeforeMount() {
  console.log('app-ygtPortal 即将被渲染')
}

function handleMount() {
  console.log('app-ygtPortal 已经渲染完成')
  store.dispatch('app/setAppLoading', false)
}

function handleUnmount() {
  console.log('app-ygtPortal 卸载了')
  store.dispatch('app/setAppLoading', false)
}

function handleError() {
  console.log('app-ygtPortal 加载出错了')
  store.dispatch('app/setAppLoading', false)
}

function handleDataChange(e) {
  // 接收子应用传递的数据 e.detail.data
  console.log('来自子应用 app-ygtPortal 的数据:', e.detail.data)
