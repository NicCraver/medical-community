...
const singleSignFuc = async () => {
  try {
    ...
    if (route.name === 'singleSign') {
      // ehr
      if (appId.value === '7686eb68fe3b4242a5991e681f865d95') {
        router.push('/app-healthFile/residentCenterInfo')
      }
      // 数据治理
      if (appId.value === '747daaf979f14c6bb332cfccb1b724cb') {
        router.push('/app-serverResourceManage/areaQuality/overview ')
      }
      // 门户
      if (appId.value === '8cc41399417548408d2c6b28c7213f4c') {
        router.push('/app-ygtPortal/standard/areaStandard')
      }
    }
    onLogin(loginDevice)
  } catch (error) {
...
const getApplicationData = (menus, basePage) => {
  ...
  // 保存菜单数据
  store.dispatch('app/setMenuData', process.env.VUE_APP_IDENTIFICATION)
  sessionStorage.setItem(`menuDataKeys${process.env.VUE_APP_IDENTIFICATION}`, JSON.stringify(tempMeuns))
  setTimeout(() => {
    spinning.value = false
