...
<script>
import { getReferralListDataDownBox } from '@/api/modules/referralList'
import { getReviewDataDownBox } from '@/api/modules/ReferralReview'
import { getAdmissionsDataDownBox } from '@/api/modules/admissions'
import { getPublicDataDownBox } from '@/api/modules/systemAdmin'
...
export default {
  ...
  computed: {
    ...
    isDisabled() {
      ...
      console.log(`flag || this.disabled`,flag || this.disabled)
      console.groupEnd()
      ...
      try {
        ...
        if (this.type === 'ICD') {
          ...
          if (this.$IS_ORI_ADMIN && this.$route.name === 'AdmissionsList') {
            ...
            this.selfValue = JSON.parse(window.sessionStorage.getItem('deptObject')).deptTypeAndDeptIds
            this.$emit('change', this.selfValue)
          }
          ...
          if (this.$IS_ORI_ADMIN && this.$route.name === 'AdmissionsList') {
            ...
            const isExit = this.optionsData.find(
              (item) => item.value === window.sessionStorage.getItem('userId'),
            )
