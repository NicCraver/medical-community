...
<template>
  <div class="admissions-list">
    <ProLayout model="tab" mainBgColor="#F5F5F5" padding="0" overflow>
      ...
      <template #main>
        <component :is="activeComponent" :activeComponent="activeComponent" />
      </template>
    </ProLayout>
  </div>
</template>
...
import Wait from "./AdmissionsWait.vue";
import Already from "./AdmissionsAlready.vue";

export default {
  components: { ProLayout, Wait, Already },
  data() {
    return {
  ...
  mounted() {
    this.activeComponent = window.sessionStorage.getItem('activeTab') || 'Wait';
    window.sessionStorage.removeItem('activeTab');
    this.$EVENT_BUS.$on('noticationEmit', () => {
      this.activeComponent = 'Wait';
    });
  },
  beforeDestroy() {
