...
function handleMicroData() {
  ...
  if (window.__MICRO_APP_ENVIRONMENT__) {
    // 主动获取基座下发的数据
    console.log('ygtPortal getData:', window.microApp.getData())
    // 页面跳转 通知 主应用 菜单切换
    ...
    window.microApp.addDataListener((data) => {
      console.log('ygtPortal addDataListener:', data)
...
function mount() {
  app = new Vue({
    router, store,
    render: (h) => h(App),
  }).$mount('#subapp-ygt-portal')
  console.log('子应用 - 微应用ygtPortal渲染了')
  handleMicroData()
}
...
function unmount() {
  ...
  app.$el.innerHTML = ''
  app = null
  console.log('子应用 - 微应用ygtPortal卸载了')
}
