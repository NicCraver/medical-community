<template>
  <div class="map">
    <el-row>
      ...
      <el-col :span="4">
        <el-input disabled v-model="lat" style="padding-left: 10px" placeholder="经度" />
      </el-col>
      <el-col :span="4">
        <el-input disabled v-model="lng" style="padding-left: 10px" placeholder="纬度" />
      </el-col>
    </el-row>
...
export default {
  ...
  computed: {
    lat() {
      let lat = '经度：'
      if (this.center[0]) {
        lat += parseFloat(this.center[0]).toFixed(2)
      }
      return lat
    },
    lng() {
      let lng = '纬度：'
      if (this.center[1]) {
        lng += parseFloat(this.center[1]).toFixed(2)
      }
      return lng
    },
  },
  ...
  methods: {
    ...
    initMap() {
      ...
      AMapLoader.load({
        ...
        .then((AMap) => {
          const center = this.center[0] && this.center[1] ? this.center : [116.3975, 39.91]
          console.log('center', this.center)
          this.mapIns = new AMap.Map('map-container', {
            zoom: this.zoom, // 初始化地图级别
            center: center, //初始化地图中心点位置
          })
          this.setMarker(center, '天安门广场')
        })
        .catch((e) => {
          console.log(e)
        })
