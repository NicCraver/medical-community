  ...
  <div class="followuped">
    <ProList class="ProList" :pageParams="pageParams" :total="total" :onInquire="onInquire">
      <template #header>
        ...
        <el-select placeholder="操作人" v-model="queryParams.followupUserId" clearable filterable>
          <el-option
        ...
        <el-select placeholder="是否超期" v-model="queryParams.overdueFlg" clearable>
          <el-option v-for="item in overdueFlgList" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      ...
      >
        ...
        <el-table-column label="操作人" prop="followupUserName" />
        ...
        <el-table-column label="操作" fixed="right" width="120">
          <template slot-scope="{ row }">
            ...
            <!-- v-if="
                ...
                row.feedbackStatus !== '/'
              " -->
            <!-- <el-button type="text" @click="pageToFollowUpDetail(row, true)">
              {{ row.followupType }}
              {{ row.feedbackStatus }}
            </el-button> -->
          </template>
