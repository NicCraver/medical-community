  ...
  <div class="AddPlanNewCom">
    <header class="flex">
      <div class="flex">
        ...
        <div :class="stepState === '1' ? 'num-text-color1' : 'num-text-color2'">基础信息</div>
      </div>
      <div class="line"></div>
      <div class="flex">
        <div>
        ...
        <div :class="stepState === '2' ? 'num-text-color1' : 'num-text-color2'">其他配置</div>
      </div>
    </header>
    <el-form label-width="90px" :rules="planDetailFormRules" :model="planDetailForm" ref="planForm" class="demo-form">
      <template v-if="stepState === '1'">
        <el-row :gutter="10">
          <el-col :span="24" class="label"> <span class="xinghao">* </span>随访病种及计划名称 </el-col>
          <el-col :span="6">
          ...
          <el-col :span="12">
            <el-form-item prop="planName" label-width="0">
              <el-input v-model="planDetailForm.planName" maxlength="20" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="18">
            <el-form-item label="开启时间" prop="followupStartTime" label-width="90px">
              <el-date-picker
                type="date"
          ...
          <el-col :span="24">
            <el-form-item label="随访频率" prop="frequencyType" label-width="90px">
              <el-radio-group v-model="planDetailForm.frequencyType" @change="typeChange">
          ...
          <el-row :gutter="10">
            <el-col :span="18">
              <el-form-item label="频率设置" prop="frequencyTimes" required>
                <el-select v-model="planDetailForm.frequencyTimes" style="width: 100%">
                  <el-option v-for="item in countList" :key="item" :label="item" :value="item" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label-width="10px" prop="frequencyUnit" required>
                <el-select v-model="planDetailForm.frequencyUnit" @change="frequencyUnitChange" style="width: 100%">
                  <el-option
        ...
        <div class="custom" v-if="planDetailForm.frequencyType === 1">
          ...
          <div class="frequency-cont">
            ...
            >
              <div class="item-top">
                <div class="item-left">
                  <el-form-item :prop="'frequency.' + index + '.sortNum'" :rules="rules.sortNum" label-width="5px">
                    <el-select
                  ...
                  <template v-if="isShowSortNum(item)">
                    <el-form-item :prop="'frequency.' + index + '.timeType'" :rules="rules.timeType" label-width="5px">
                      <el-select
                        v-model="item.timeType"
                        style="width: 135px"
                        :style="{
                          'margin-right': item.timeType === 'customTime' ? '148px' : 0,
                        }"
                      ...
                      >
                        <el-option
                          ...
                          :value="val.value"
                          :disabled="
                            (val.value === 'afterLastTime' && !index) || (val.value === 'afterStartTime' && index > 0)
                          "
                        />
                      </el-select>
                    </el-form-item>
                    <template v-if="item.timeType === 'afterStartTime' || item.timeType === 'afterLastTime'">
                      <el-form-item
                  ...
                  <template v-else>
                    ...
                    >
                      <el-select
                        ...
                        style="width: 65px"
                      >
                        <el-option v-for="val in countList" :key="val" :label="val" :value="val" />
                      </el-select>
                    </el-form-item>
                    <el-form-item
                  ...
                  >
                    <div
                      class="selectDate"
                      v-if="item.timeType === 'afterStartTime' || item.timeType === 'afterLastTime'"
                    >
                      <span class="selectDateCls" v-if="item.followupDateStr">
                        {{ formatDate(item.followupDateStr, 'MM月DD日') }}
                      </span>
                      <span class="selectDateCls warn-btn" v-else-if="item.isValied"> 请重新填写 </span>
                      <span class="selectDateCls noSelectDate" v-else> 请生成日期 </span>
                    </div>
                    <div class="selectDate selectDateCusor" v-else-if="item.timeType === 'customTime'">
                      <el-popover placement="right" width="400" trigger="click">
                        ...
                        <el-calendar class="calendar">
                          <template slot="dateCell" slot-scope="{ data, date }">
                            ...
                            >
                              {{ formatDate(date, 'DD') }}
                              <div class="labelCls" v-if="getDateContent(date, index)">
                                {{ getDateContent(date, index) }}
                              </div>
                            </div>
                          </template>
                        </el-calendar>
                        <el-button slot="reference" type="text" class="btn-text">
                          <span
                          ...
                          <span v-else-if="item.isTemp || item.isValied" class="selectDateCls warn-btn">
                            请重新填写
                          </span>
                ...
                <div class="item-right">
                  <el-button type="text" :disabled="!isShowSortNum(item)" @click="addFuc(item, index)">
                    <img src="@/assets/add_point.png" alt="" />
                  </el-button>
                  <el-button
                    type="text"
                    :disabled="index === 0 && planDetailForm?.frequency?.length === 1"
              ...
              <div class="item-bottom" v-if="planDetailForm.isCustomTable === '1'">
                <el-form-item
          ...
          <div class="template-bar">
            <el-button plain size="mini" class="muban-btn" @click="saveTempFuc">存为模板 </el-button>
            <div class="text">
              <span>
                总计：

                <template v-if="allFollowupTimes"> {{ allFollowupTimes }}次随访 </template>
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="18">
            <el-form-item label="随访方式" prop="followupType">
              <el-select v-model="planDetailForm.followupType" style="width: 100%" @change="handleFollowupChange">
                <el-option v-for="item in followUpTypeList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="18" v-if="planDetailForm.frequencyType === 0 || planDetailForm.isCustomTable === '0'">
          ...
          <el-col :span="18">
            <el-form-item label="患者提醒" prop="msgToRemind">
              <el-checkbox-group v-model="planDetailForm.msgToRemind">
                ...
                <el-checkbox
                  label="2"
                  :disabled="planDetailForm.followupType === '1' || planDetailForm.followupType === '3'"
                  >短信消息</el-checkbox
                >
              </el-checkbox-group>
            </el-form-item>
              ...
              <el-checkbox-group v-model="planDetailForm.systemCall">
                ...
                >
                  ...
                  <el-tooltip
                    ...
                    v-if="item.label === '提前'"
                  >
                    <i class="el-icon el-icon-question" style="color: #446bbd"></i>
                  </el-tooltip>
                </el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          ...
          <el-col :span="18">
            <el-form-item label="" prop="msgToUserId" label-width="90px">
              ...
              <el-select v-model="planDetailForm.msgToUserId" multiple style="width: calc(100% - 90px)">
                <el-option
          ...
          <el-col :span="18">
            <el-form-item label="超期编辑" prop="beyondEditor">
              ...
              >
                ...
                <el-radio label="0" :disabled="planDetailForm.systemCall && planDetailForm.systemCall.includes('3')"
                  >关</el-radio
                >
              </el-radio-group>
            </el-form-item>
    ...
    <div class="footer" v-if="followupType === 'single'">
      ...
      <div class="right-btn">
        <el-button plain class="step-btn" v-if="stepState === '1'" @click="nextFuc"> 下一步 </el-button>
        <el-button plain class="step-btn" v-if="stepState === '2'" @click="beforeFuc"> 上一步 </el-button>
        <el-button type="primary" v-if="stepState === '2'" @click="addPlanSubmitFuc"> 创建 </el-button>
      </div>
    </div>
...
import { followUpTypeList, warnList, planTypeList, unitList, frequencyUnitList, timeTypeList } from '@/utils/data-map'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      pickerOptions: {
        disabledDate(time) {
          return Date.now() > time.getTime() + 24 * 60 * 60 * 1000 || time.getDay() === 6 || time.getDay() === 0
        },
      },
      planDetailFormRules: {},
      // 除自定义项中的其他rule
      ruleFixed: {
        diseaseCode: [{ required: true, message: '请选择随访病种', trigger: 'change' }],
        planName: [{ required: true, message: '请输入随访计划', trigger: 'change' }],
        planType: [{ required: true, message: '请选择计划类型', trigger: 'change' }],
        followupStartTime: [{ required: true, message: '请选择开启日期', trigger: 'change' }],
        startTimeStr: [{ required: true, message: '请选择开启时间', trigger: 'change' }],
        frequencyType: [{ required: true, message: '请选择随访频率方式', trigger: 'change' }],
        followupHosId: [{ required: true, message: '请选择随访机构', trigger: 'change' }],
        followupDeptId: [{ required: true, message: '请选择随访科室', trigger: 'change' }],
        followupType: [{ required: true, message: '请选择随访方式', trigger: 'change' }],
        followupTable: [{ required: true, message: '请选择随访表单', trigger: 'change' }],
        msgToRemind: [{ validator: msgToRemindValodator, trigger: 'change' }],
        systemCall: [{ required: true, message: '请选择医生提醒', trigger: 'change' }],
        msgToUserId: [{ required: true, message: '请选择提醒范围', trigger: 'change' }],
        beyondEditor: [{ required: true, message: '请选择超期编辑', trigger: 'change' }],
        // 固定频率
        frequencyTimes: [{ validator: frequencyTimesValodator, trigger: 'change' }],
        frequencyUnit: [{ validator: frequencyUnitValodator, trigger: 'change' }],
        followupSize: [{ validator: followupSizeValodator, trigger: 'change' }],
      },
      // 自定义rule
      rules: {
        // 自定义
        sortNum: [{ required: true, message: '请选择第几次', trigger: 'change' }],
        timeType: [{ required: true, message: '请选择日期计算规则', trigger: 'change' }],
        followupDateStrItem: [{ required: true, message: '请选择日期', trigger: 'change' }],
        timeContent: [{ required: true, message: '请选择第几天', trigger: 'change' }],
        frequencyTimesItem: [{ required: true, message: '请选择执行间隔', trigger: 'change' }],
        frequencyUnitItem: [{ required: true, message: '请选择执行单位', trigger: 'change' }],
        followupSizeItem: [{ required: true, message: '请选择执行频率', trigger: 'change' }],
        followupTableItem: [{ required: true, message: '请选择随访表单', trigger: 'change' }],
      },
  ...
  methods: {
    ...
    handleDiseaseChange(diseaseCode) {
      ...
      let diseaseName = this.diseaseTypeList.find((item) => item.richDiseaseCode === diseaseCode).richDiseaseName
      this.planDetailForm.planName = `${dayjs(new Date()).format('YYYY-MM')}${diseaseName}随访计划`
    },
    ...
    calcAgain() {
      ...
      frequency.forEach((item, index) => {
        if (item.timeType !== 'customTime') {
          ...
          let lastDate = dayjs(frequency[index - 1].followupDateStr).format('YYYYMMDD')
    ...
    followupTimeChange() {
      if (this.planDetailForm.followupStartTime && this.planDetailForm.startTimeStr) {
    ...
    typeChange(val) {
      if (val === 1 && !this.planDetailForm.followupStartTime) {
        // 校验开启时间
        this.$refs['planForm'].validateField(['followupStartTime', 'startTimeStr'], (valid) => {})
    ...
    sortNumChange(val, item, index) {
      ...
      if (str?.indexOf('-') > -1) {
        this.$set(this.planDetailForm.frequency[index], 'timeType', 'customTime')
      }
    },
    // 自开始日期还是自选日期change
    ...
    validatecDateFuc(item, index) {
      ...
      let lastItemDate = index > 0 ? frequency[index - 1]?.followupDateStr || '' : ''
      let lastItemDateTime = lastItemDate ? new Date(lastItemDate).getTime() : ''
      ...
      let nextItemDateTime = newNextItemDateTimes?.length ? newNextItemDateTimes[0] || '' : ''
      ...
      if (lastItemDateTime && followupDateStrTime <= lastItemDateTime) {
        ...
        // this.$set(frequency[index], "followupDateStr", "");
        this.$confirm('您设定的本次随访时间与其它随访事件有冲突，请确认。', '提示', {
    ...
    frequencyUnitChange(val, item, index) {
      ...
      if (index !== undefined) {
        if (Number(item.frequencyTimes) > this.countList) {
          this.$set(this.planDetailForm.frequency[index], 'frequencyTimes', this.countList)
        }
      } else {
    ...
    setDateObj(nowDate, index) {
      ...
      let lastDateDetailOfMonth = Math.floor(dayjs(nowDate).endOf('month').date() / 2) //月中几号
      let str1 = ''
      let code1 = ''
      ...
      // debugger
      if (WeekOfMonth === 1 || (WeekOfMonth === 2 && calcWeek < WeekStartDayOfMonth)) {
        ...
        str3 = `(月)第${intToChinese(WeekOfMonth)}周的周${week == 0 ? '日' : intToChinese(week)}`
    ...
    resetObjFuc(item, index, type) {
      ...
      let newFrequency = frequency.map((vv, kk) => {
        ...
        if (type === 'add') {
          ...
          if (kk >= index) {
            this.$delete(this.dateObj, index)
            if (kk + 1 < frequency.length && frequency[kk + 1].timeType === 'customTime') {
        ...
        if (kk > index) {
          if (type === 'add') {
            return {
              ...vv,
              sortNum: this.isShowSortNum(vv) ? num + 1 : `${num + 1}-${num + 1}`,
            }
          } else if (type === 'delete') {
            return {
              ...vv,
    ...
    handleSubmitData() {
      ...
      let followupDeptName = this.deptList.find((item) => item.deptId === planDetailForm.followupDeptId)?.deptName
      ...
      params = {
        ...
        followupEndTime: followupTime?.length ? followupTime[followupTime.length - 1] : '',
    ...
    handleCustomData() {
      ...
      let frequencyDtoList = frequency.map((item, index) => {
        ...
        if (this.isShowSortNum(item)) {
          // 前几次
          if (item.timeType === 'afterStartTime' || item.timeType === 'afterLastTime') {
            obj = {
              sortNum: item.sortNum,
              timeType: item.timeType,
    ...
    getDateCls(date, item, index) {
      ...
      let arr1 = frequency.map((val, key) => {
        if (key < index) {
          return val.followupDateStr ? new Date(val.followupDateStr).getTime() : 0
        } else {
          return 0
        }
      })
      ...
      if (
        ...
        (arrNew1?.length && datesTimes <= arrNew1[arrNew1.length - 1] && arrNew2?.length && datesTimes >= arrNew2[0])
      ) {
    ...
    */
    calcFollowupTime(item) {
      if (!item?.followupDateStr || !item?.frequencyTimes || !item?.frequencyUnit) {
        return
      }
      ...
      if (!lastDateType || lastDateType.indexOf('day_') > -1) {
        switch (item.frequencyUnit) {
          case '1':
            followupDateStr = dayjs(lastTime).add(Number(item.frequencyTimes), 'day').format('YYYY-MM-DD')
            break
          case '2':
            followupDateStr = dayjs(lastTime).add(Number(item.frequencyTimes), 'month').format('YYYY-MM-DD')
            break
          case '3':
            followupDateStr = dayjs(lastTime).add(Number(item.frequencyTimes), 'year').format('YYYY-MM-DD')
            break
        }
        ...
        switch (item.frequencyUnit) {
          case '1':
            ...
            break
          case '2':
            followupDate = dayjs(lastTime).add(Number(item.frequencyTimes), 'month')
            lastDateDetailOfMonth = Math.floor(dayjs(followupDate).endOf('month').date() / 2)
            followupDateStr = dayjs(followupDate).format('YYYY-MM') + '-' + lastDateDetailOfMonth
            break
          case '3':
            followupDate = dayjs(lastTime).add(Number(item.frequencyTimes), 'year')
            lastDateDetailOfMonth = Math.floor(dayjs(followupDate).endOf('month').date() / 2)
            followupDateStr = dayjs(followupDate).format('YYYY-MM') + '-' + lastDateDetailOfMonth
            break
        }
        ...
        switch (item.frequencyUnit) {
          ...
          case '2':
            ...
            let WeekStartOfMonth = dayjs(lastTime).add(Number(item.frequencyTimes), 'month').startOf('month')
          ...
          case '3':
            ...
            let WeekStartOfMonth3 = dayjs(lastTime).add(Number(item.frequencyTimes), 'year').startOf('month')
        ...
        switch (item.frequencyUnit) {
          ...
          case '2':
            ...
            let calcWeek2 = week2 == 0 ? 7 : week2
            let WeekEndOfMonth = dayjs(lastTime).add(Number(item.frequencyTimes), 'month').endOf('month')
            let WeekEndDayOfMonth = dayjs(WeekEndOfMonth).day()
            let calcWeekEndDayOfMonth = WeekEndDayOfMonth == 0 ? 7 : WeekEndDayOfMonth
            ...
            followupDateStr = dayjs(WeekEndOfMonth).subtract(subtractNum, 'day').format('YYYY-MM-DD')
            break
          case '3':
            let week3 = Number(lastDateType.split('lastWeek_0_')[1])
            let calcWeek3 = week3 == 0 ? 7 : week3
            let WeekEndOfMonth3 = dayjs(lastTime).add(Number(item.frequencyTimes), 'year').endOf('month')
            let WeekEndDayOfMonth3 = dayjs(WeekEndOfMonth3).day()
            let calcWeekEndDayOfMonth3 = WeekEndDayOfMonth3 == 0 ? 7 : WeekEndDayOfMonth3
            ...
            followupDateStr = dayjs(WeekEndOfMonth3).subtract(subtractNum3, 'day').format('YYYY-MM-DD')
            break
        }
        ...
        switch (item.frequencyUnit) {
          ...
          case '2':
            ...
            let secondWeekStartDay = dayjs(WeekStartOfMonth).startOf('month').date(num) // 获取当前月份的 8 号
          ...
          case '3':
            ...
            let secondWeekStartDay3 = dayjs(WeekStartOfMonth3).startOf('month').date(num3) // 获取当前月份的 8 号
    ...
    handleApplyTemp(result) {
      ...
      let arr = result.map((item, index) => {
        ...
        let obj = {
          sortNum: item.type === '1' ? `${item.sortNum}-${item.sortNum}` : Number(item.sortNum),
          timeType: item.timeType || '',
      ...
      if (isTooltip) {
        this.$message({
          type: 'warning',
          dangerouslyUseHTMLString: true,
          message: "<span style='color: #191919'>引用模版，请再次确认<i style='color: #4468BD'>描紫</i>日期</span>",
        })
      }
    },
    quoteTempClose() {
