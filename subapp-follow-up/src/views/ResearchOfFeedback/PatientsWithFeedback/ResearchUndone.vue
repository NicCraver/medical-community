    ...
    <ProList class="ProList" :pageParams="pageParams" :total="total" :onInquire="onInquire">
      <template #header>
        ...
        <el-select placeholder="纳入人" v-model="queryParams.IncludeUserId" clearable>
          <el-option v-for="item in followupIncludeUserList" :key="item.userInfoId" :label="item.loginName" :value="item.userInfoId" />
        </el-select>
        <el-select placeholder="发起人" v-model="queryParams.researchUserId" clearable filterable>
          <el-option v-for="item in researchUserList" :key="item.researchUserId" :label="item.researchUserName" :value="item.researchUserId" />
        </el-select>
        <OrgHosSelect ref="orgRef" v-model="pageParams.orgId" placeholder="集团"></OrgHosSelect>
        <OrgHosSelect ref="hosRef" v-model="pageParams.hosId" :parentId="pageParams.orgId" placeholder="调研机构"></OrgHosSelect>
        <el-date-picker type="daterange" value-format="yyyy-MM-dd" start-placeholder="纳入开始日期" end-placeholder="纳入结束日期" range-separator="至" v-model="includeDate" clearable style="width: 410px" />
        <el-date-picker type="daterange" value-format="yyyy-MM-dd" start-placeholder="完成开始日期" end-placeholder="完成结束日期" range-separator="至" v-model="finishDate" clearable style="width: 410px" v-if="activeResearchStatus === '2'" />
      </template>
      <template #actions>
      ...
      <el-table v-adaptive="{ bottomOffset: 50 }" height="0" ref="singleTable" :data="dataList" border v-loading="loading" row-key="id" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" fixed="right" width="120">
          <template slot-scope="{ row }">
            <el-button type="text" v-if="activeResearchStatus === '2'" @click="pageToResearchDetail(row)">查看</el-button>
          ...
          if (res.result && res.result.length) {
            this.dataList = res.result.map((item) => ({
              ...
              followUpDiseaseText: item.followUpDiseaseCodes
                .map(
                  (code) =>
                    this.diseaseTypeList.find(
                      (diseaseType) => diseaseType.value === code
                    ).label
                )
                .join(","),
            }));
