      ...
      <ProTable>
        ...
        <el-table ref="table" tooltip-effect="light" height="0" v-adaptive="{ bottomOffset: 105 }" v-loading="loading" :data="tableData" @selection-change="handleSelectionChange" border stripe>
          ...
          <el-table-column label="规则类型" prop="type">
            <template>{{proEnv==='heilongjiang'?'时效性':'及时性'}}</template>
          </el-table-column>
          ...
          <el-table-column label="业务类型">
            <template slot-scope="{row}">{{row.businessTableName.indexOf(',') === -1 && row.businessTableName == row.targetTableName ? '单张表':'多张表'}}</template>
          </el-table-column>
...
import Add from "./TimelinessAdd.vue";
      ...
      if (this.multipleSelection.length === 0) {
        ...
        this.$confirm("是否确认删除选中的数据?", "提示", {
          ...
          .then(() => {
            ...
            //   if (this.multipleSelection[i].enableStatus == 1) {
